...
public class NumberUtil {
     ...
     * 最大的字节长度
     **/
    private static final int MAX_INT_BYTE_SIZE = Integer.SIZE / 8;

    /**
     * 最大的字节长度
     **/
    private static final int MAX_LONG_BYTE_SIZE = Long.SIZE / 8;
    ...
    public static long bytesToNumber(byte[] value, int startIndex, int endIndex, int maxLongByteSize) {
        ...
        if (value[lastPosition] < 0 && (endIndex - startIndex) < maxLongByteSize) {
            result -= (long) Math.pow(2, 8 * (endIndex - startIndex));
        }
        return result;
    }
