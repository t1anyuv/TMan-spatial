 ...
 * @author hty
 */
public class BMTreeRLOrderGenerator {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("========================================");
            System.out.println("BMTree RLOrder生成器");
            System.out.println("========================================");
            System.out.println("输入文件: " + originalRLOrderPath);
            System.out.println("输出文件: " + outputPath);
            System.out.println("BMTree配置: " + bmtreeConfigPath);
            System.out.println("Bit长度: " + bmtreeBitLength);
            System.out.println("分辨率: " + resolution);
            System.out.println("========================================\n");
            
            // 解析bit长度
            int[] bitLength = parseBitLength(bmtreeBitLength);
            System.out.println("解析bit长度: [" + bitLength[0] + ", " + bitLength[1] + "]");
            System.out.println("网格大小: " + (1 << bitLength[0]) + " x " + (1 << bitLength[1]) + "\n");
            ...
            // 步骤2：计算BMTree SFC值
            System.out.println("[步骤2] 计算BMTree SFC值...");
            calculateBMTreeSFCValues(quads, bmtreeConfigPath, bitLength, resolution);
     ...
     * 解析bit长度字符串
     */
    private static int[] parseBitLength(String bitLengthStr) {
    ...
    /**
     * 步骤2：计算每个quad的BMTree SFC值
     */
    public static void calculateBMTreeSFCValues(
        List<QuadInfo> quads,
        ...
        if (meta != null && meta.boundary != null) {
            globalXmin = meta.boundary.getXMin();
            globalXmax = meta.boundary.getXMax();
            globalYmin = meta.boundary.getYMin();
            globalYmax = meta.boundary.getYMax();
            System.out.println("  - 从metadata读取全局边界");
        } else {
            // 备选：从quads计算
            globalXmin = quads.stream().mapToDouble(q -> q.xmin).min().orElse(0);
            globalXmax = quads.stream().mapToDouble(q -> q.xmax).max().orElse(0);
            globalYmin = quads.stream().mapToDouble(q -> q.ymin).min().orElse(0);
            globalYmax = quads.stream().mapToDouble(q -> q.ymax).max().orElse(0);
            System.out.println("  - 从quads计算全局边界");
        }
        ...
        // 初始化BMTree索引
        BMTreeIndex bmtreeIndex = BMTreeIndex.apply(
            (short) resolution,
        ...
        System.out.println("  - BMTree索引初始化完成");
        ...
        for (QuadInfo quad : quads) {
            ...
            // 调用BMTree索引计算SFC值
            Tuple3<Object, Object, Object> result = bmtreeIndex.index(geo, false);
    ...
    public static Map<Long, Integer> assignNewOrders(List<QuadInfo> quads) {
        ...
        for (int i = 0; i < quads.size(); i++) {
            ...
            quad.newOrder = i;  // 新的order就是排序后的索引（0-1689）
    ...
    public static void generateRLOrderBMTree(
        ...
        metadata.put("version", "2.0-BMTree");
