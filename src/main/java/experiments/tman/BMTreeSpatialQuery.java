 ...
 * BMTree索引空间查询实验程序
 * <p>
 * 使用BMTree (Binary Multi-dimensional Tree) 索引执行空间范围查询
 * <p>
 * 命令行参数：
 * [0] tableName - HBase表名
 ...
 * <p>
 * 示例：
 * java experiments.tman.BMTreeSpatialQuery \
 * bmtree_table \
 * "116.1,39.6,116.5,40.0;116.2,39.7,116.6,40.1" \
 * /results/bmtree_query_results.csv
 * <p>
 * 注意：
 * - BMTree索引使用minSFC作为RowKey进行范围查询
 * - 查询性能取决于BMTree结构（best_tree.txt）和查询窗口大小
 * - 第一个查询用于预热，不计入统计
 */
public class BMTreeSpatialQuery extends SpatialQuery {
    ...
    public List<FilterBase> getFilters(String condition, TableConfig tableConfig) {
        ...
        // 计算所有范围中的最小 SFC 值（用于 SpatialWithSFC 过滤器）
        long queryMinSFC = Long.MAX_VALUE;
        for (org.locationtech.sfcurve.IndexRange range : ranges) {
            queryMinSFC = Math.min(queryMinSFC, range.lower());
        }
        if (ranges.isEmpty()) {
            queryMinSFC = 0L;
        }
        ...
        System.out.println(String.format(
                "[BMTreeSpatialQuery] 查询窗口: [%.4f, %.4f] - [%.4f, %.4f]",
                env.getXMin(), env.getYMin(), env.getXMax(), env.getYMax()));
        System.out.println(String.format(
                "[BMTreeSpatialQuery] 索引范围数: %d, 第一个范围: [%d, %d]",
                ranges.size(),
                ranges.isEmpty() ? 0 : ranges.get(0).lower(),
                ranges.isEmpty() ? 0 : ranges.get(0).upper()));
