 ...
 * <p>
 * 示例：
 * java experiments.tman.LMSFCSpatialQuery \
 * lmsfc_table \
...
public class LMSFCSpatialQuery extends SpatialQuery {
    ...
    public List<FilterBase> getFilters(String condition, TableConfig tableConfig) {
         ...
         */
        // 5. 使用查询分割算法计算索引范围
        java.util.List<org.locationtech.sfcurve.IndexRange> ranges = lmsfcIndex.rangesWithSplit(
                env.getXMin(), env.getYMin(), env.getXMax(), env.getYMax(), 3);
        ...
        System.out.println(String.format(
                ...
                env.getXMin(), env.getYMin(), env.getXMax(), env.getYMax()));
        System.out.println(String.format(
                "[LMSFCSpatialQuery] 查询分割后范围数: %d, 第一个范围: [%d, %d]",
                ranges.size(), ranges.get(0).lower(), ranges.get(0).upper()));
