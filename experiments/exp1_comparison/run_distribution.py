"""
Exp 1b — Comparative Experiment: Varying Query Distribution

Fix query range to default (500 m), vary spatial distribution:
  UNI (uniform) | SKE (skewed) | GUS (Gaussian)

Compare: LETI vs TShape vs XZ* vs LMSFC vs BMTree
...
class DistributionExperiment(BaseExperiment):
    ...
    def run(self):
        ...
        for dataset in datasets:
            ...
            print(f"\n[Exp1b] Loading segments from {seg_path} ...")
            segments = load_segments(seg_path)

            methods = get_methods(cfg)
            print(f"[Exp1b] Building indexes for dataset={dataset} ...")
            for mname, method in methods.items():
                print(f"  Building {mname} ...")
                method.build(segments)

            for dist in tqdm(dists, desc=f"{dataset} distribution sweep"):
...
if __name__ == "__main__":
    ...
    DistributionExperiment(args.config).run()
